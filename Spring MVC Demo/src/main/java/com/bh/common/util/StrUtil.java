...
public class StrUtil {
    ...
    public static Boolean in(String value,String ins){
        String[] aryStr = ins.split(",");
        for (int i = 0 ; i <aryStr.length ; i++ ) {
            if(aryStr[i].equals(value)){
                return true;
            }
        }
        return false;
    ...
    取得固定长度的数值
     */
    public static String IntegerToLength(int key,int length){
        String keyStr = String.valueOf(key);
        String result = keyStr;
        for (int i=0;i<keyStr.length() - length;i++){
            result += "0" + result;
        }
        return result;
    }
